...
gdf_combined['norm_val'] = (gdf_combined['VAL_AUTO_NUM'] - gdf_combined['VAL_AUTO_NUM'].min()) / (gdf_combined['VAL_AUTO_NUM'].max() - gdf_combined['VAL_AUTO_NUM'].min())

# Índice composto: média ponderada
# Pesos: 30% Tipo, 30% Gravidade, 40% Valor (ajuste conforme necessário)
gdf_combined['indice_risco'] = (0.3 * gdf_combined['norm_tipo'] + 
                                 0.3 * gdf_combined['norm_grav'] + 
                                 0.4 * gdf_combined['norm_val']) * 100
...
agg_municipio = gdf.groupby('MUNICIPIO').agg({
    'VAL_AUTO_NUM': ['count', 'sum', 'mean', 'max'],
    'TIPO_AUTO': lambda x: x.value_counts().index[0] if len(x) > 0 else '',
    'geometry': 'first'
}).reset_index()

agg_municipio.columns = ['MUNICIPIO', 'qtd_infraçoes', 'valor_total', 'valor_medio', 'valor_maximo', 'tipo_principal', 'geometry']
...
output_combined.to_file(heat_dir / "heat_indice_composto.geojson", driver='GeoJSON')
